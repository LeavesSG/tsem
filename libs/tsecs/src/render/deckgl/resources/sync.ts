import { QueryEC, Res, type Commands, type Component, type Entity } from "../../../core.ts";
...
export class BjsSynchronizer extends Res {
    cmd: Commands;
    syncMap: Pool<typeof Component<Scalar>, SyncFunc>;
    ...
    constructor(cmd: Commands) {
        ...
        this.syncMap = new Pool<typeof Component<Scalar>, SyncFunc>(cmd);
    ...
    private bind(comps: Component[], name?: Name) {
        ...
        const __name = name ?? comps.find((comp) => comp instanceof Name);
        if (!__name || !__name.buf.name) return;
    ...
    addSyncMap(
        record: Iterable<[typeof Component<Scalar>, SyncFunc]>,
    ) {
